...
export const getNotify = async (req, res) => {
  try {
    const notify = await Notify.find().sort({ createAt: -1 });
...
export const updateNotify = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status(404).send("no notify with that id");
  try {
    const updateNotify = await Notify.findByIdAndUpdate(
      { _id: id },
      { ...notify, _id: id },
      { new: true }
    );
...
export const seenNotify = async (req, res) => {
  ...
  try {
    ...
    const index = await notify.seen.findIndex(
      (id) => id === String(req.userId)
    );
    if (index === -1) {
      notify.seen.push(req.userId);
    }
    const updateNotify = await Notify.findByIdAndUpdate({ _id: id }, notify, {
      new: true,
    });
    return res.json({ result: updateNotify });
  } catch (error) {
...
export const deleteNotify = async (req, res) => {
  ...
  if (!req.userId) return res.status(403).send("unauthenticated");
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status.send("no notify with that id");
  try {
