...
export const createAdvertise = async (req, res) => {
  ...
  const image = filename.map((img) => img.filename);
  const newAdvertise = new Advertises({ ...advertise, image: image });
  try {
...
export const updateAdvertise = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status(404).send("no advertise with that id");
  try {
    const updateAdvertise = await Advertises.findByIdAndUpdate(
      { _id: id },
      { ...advertise, _id: id },
      { new: true }
    );
...
export const deleteAdvertise = async (req, res) => {
  const id = req.params.id;
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status.send("no advertise with that id");
  try {
