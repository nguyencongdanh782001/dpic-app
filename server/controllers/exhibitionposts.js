...
export const getPrivateExhposts = async (req, res) => {
  try {
    const exhposts = await ExhibitionPosts.find({ creator: req.userId }).sort({
      createdAt: -1,
    });
...
export const createExhposts = async (req, res) => {
  ...
  const newExhPost = new ExhibitionPosts({
    ...exhpost,
    creator: req.userId,
    image:image,
    approve: false,
...
export const updateExhposts = async (req, res) => {
  ...
  try {
    const updateExhPost = await ExhibitionPosts.findByIdAndUpdate(
      { _id: id },
      { ...exhpost, _id: id },
      { new: true }
    );
...
export const likeExhpost = async (req, res) => {
  ...
  try {
    ...
    const index = await exhpost.like.findIndex(
