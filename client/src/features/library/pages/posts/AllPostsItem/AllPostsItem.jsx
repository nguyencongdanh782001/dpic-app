    ...
    if (length === 1 || length === 0) {
      return (
        <>
          {posts?.image?.map(
            (images, index) =>
              index === current && (
                <Image key={index} src={`${process.env.REACT_APP_API_IMAGE}${images}`} alt="" />
              )
          )}
        </>
      );
      ...
      return (
        <>
          ...
          {posts?.image?.map(
            (images, index) =>
              index === current && <Image key={index} src={`${process.env.REACT_APP_API_IMAGE}${images}`} alt="" />
          )}
        </>
      );
    }
  ...
  return (
    <Card>
        ...
        <ClickAwayListener onClickAway={handleClickAway}>
          <MoreMenu>
            ...
            {open ? (
              <MoreMenuItem>
                <MoreMenuContent
                  onClick={handleCurrentId.bind(this, posts._id)}
                >
      ...
      <CardContent>
        ...
        <Typography variant="subtitle1">{posts?.camera}</Typography>
      ...
      <CardActionButton>
        ...
        >
          <TextsmsOutlinedIcon /> &nbsp; {posts?.comment?.length}
        </ButtonBottom>
        <ButtonBottom
          disabled={!user?.role}
          size="large"
